...
function createInitialParticles() {
	...
	for (x = 50; x < _canvasWidth - 50; x += 25) {
		...
		_particles.push(new Particle(x - _canvasHalfWidth, halfHeight - 75 + (Math.random() * 100)));
	}
}
...
function redraw() {
	...
	if (_particles.length < MAX_PARTICLES && _mouseX && _mouseY) {
		...
		_particles.push(new Particle(_mouseX - _canvasHalfWidth, _mouseY));
		_mouseX = false;
		_mouseY = false;
	}
		...
		for (j = i + 1; j < _particles.length; j++) {
			...
			if (distance < RADIUS) {
				...
				if (lines <= MAX_LINES) {
					...
					_context.beginPath();
					_context.moveTo(particleA.getX() + _canvasHalfWidth, particleA.getY());
					_context.lineTo(particleB.getX() + _canvasHalfWidth, particleB.getY());
